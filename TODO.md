...
## projects/project-01.html(Eco Guardians(仮))

- [x] タイトル・一言ピッチ・メタ情報(ジャンル/担当/使用ツール)を更新済み
- [x] 制作プロセス4ステップの説明文を更新済み
- [x] 企画の背景・工夫した点・学んだこと/今後の課題を更新済み
- [x] スクリーンショットのキャプション文を更新済み(画像自体は未配置)
- [ ] 制作期間・制作人数が `[制作期間を入力]` `[チーム人数を入力]` のまま(確定したら差し替え)
- [ ] Play映像:YouTube動画を埋め込むか `assets/videos/project-01/` に動画ファイルを配置してiframe/videoタグを有効化
- [ ] Unity WebGLビルド:`assets/unity-builds/project-01/` にビルド一式を配置してiframeを有効化
- [ ] itch.ioボタンのリンクが `href="#"` のまま
- [ ] 資料セクションのPDF3点(アイディア書/企画書/試作書)を `assets/docs/project-01/` に配置
- [ ] スクリーンショット画像本体を `assets/images/project-01/` に配置し、gallery-itemを`<img>`タグに差し替え
...
1. 制作期間・制作人数が決まり次第、両プロジェクトページのメタ情報を更新
2. Play映像・スクリーンショット・Unityビルドなど実ファイルを用意でき次第、対応するフォルダに配置してタグを有効化
3. X・itch.io・GitHubのアカウントが決まり次第、index.htmlの連絡先リンクを更新
4. 資料(企画書・アイディア書・試作書)のPDFを用意でき次第 `assets/docs/` 配下に配置
